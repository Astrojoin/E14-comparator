...
function buildReport(groups, anomalies) {
  ...
  lines.push("  Duplicados con <>    " + padLeft(anomalies.length, 4) + (anomalies.length > 0 ? "  <-- revisar" : ""));
  ...
  for (const group of sortedGroups) {
    ...
    const tag = hasAnomaly ? "<>" : "OK";
    const dupCount = group.duplicates.length;
    const anomalyCount = group.duplicates.filter(d => d.hash !== group.hash).length;

    lines.push("  [" + tag + "] " + shortHash(group.hash) + ".pdf  (" + dupCount + " dup" + (anomalyCount > 0 ? ", " + anomalyCount + " con hash <>" : "") + ")");
